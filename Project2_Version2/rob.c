...
ROB_entry_node * ROB_push(ROB_entry entry)
{
    if (ROB_is_full())
        return NULL;
...
int rob_head_peek()
{
    ...
    if (ROB_is_empty() || !rob->head->entry.status || (!rob->head->entry.mready && (strcmp(rob->head->entry.opcode_str, "LOAD") == 0 || strcmp(rob->head->entry.opcode_str, "LDR") == 0 || strcmp(rob->head->entry.opcode_str, "STORE") == 0 || strcmp(rob->head->entry.opcode_str, "STR") == 0)))
        return 0;
    return 1;
}
...
void ROB_squash_after(ROB_entry_node *sch_node)
{
    {
        if (ROB_is_empty())
            return;
