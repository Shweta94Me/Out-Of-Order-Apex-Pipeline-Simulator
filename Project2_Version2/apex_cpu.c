...
node_attr createData(APEX_CPU *cpu)
{
    ...
    // Bahaar :: Saving pointer to ROB_Entry_node, for JBU
    //         JBU needs pointer to ROB_Entry_node in order to squash BIS and ROB Entries
    data.rob_entry = cpu->decode.rob_entry;
    ...
    while (temp && !isQueueEmpty())
    {
		...
		*/
        ...
        if(temp->data.FU_Type == JBU_FU && !cpu->jbu1.has_insn && temp->data.rs1_ready && temp->data.rs2_ready && temp->data.rs3_ready)
        {
            ...
            // Bahaar :: Pointer to ROB_Entry_node is needed by JBU_FU, in order to
            //          squash instructions in ROB and BIS
            cpu->jbu1.rob_entry = temp->data.rob_entry;
...
/*Add entry to ROB, and save pointer to ROB Entry, to be used by JBU to squash, later*/
void add_instr_to_ROB(APEX_CPU *cpu, int mready)
{
    ROB_entry data = create_ROB_data(cpu, mready);
    ROB_entry_node *entry = ROB_push(data);

    // Bahaar :: Saving pointer to ROB_Entry_node, for JBU
    // JBU needs pointer to ROB Entry in order to squash BIS and ROB Entries
    cpu->decode.rob_entry = entry;
}
...
void dispatch_instr_to_IQ(APEX_CPU *cpu, enum FU fu_type)
{
    ...
    else if (fu_type == JBU_FU)
    {
        ...
        if (phy_reg_dest != -1)
        {
            ...
            // adding instruction to rob
            // -- calling this first, since it saves a pointer to ROB_Entry_node in cpu->decode,
            //     required by JBU_FU to squash BIS and ROB entries
            add_instr_to_ROB(cpu,0);
